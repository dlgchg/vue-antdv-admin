 ...
 * @LastEditTime: 2020-08-11 14:07:33
...
<template>
  <!-- <div>
    <template v-if="!item.hidden"> -->
      ...
      <Submenu v-else-if="!item.hidden" :name="item.path">
        <template slot="title">
          <Icon :type="item.meta ? item.meta.icon : 'ios-help-circle'" />
          <span>{{ item.meta && item.meta.title }}</span>
        </template>
        <sider-item v-for="child in item.children" :key="child.path" :item="child" :base-path="resolvePath(item.path)" />
      </Submenu>
    <!-- </template>
  </div> -->
</template>
